...
function getRemoteConfig(): RemoteConfig | null {
  const host = process.env.RUNBROWSER_HOST || process.env.RUNBROWSER_HOST
  if (!host) {
    return null
  }
  return {
...
/** Resolve a resource file from our own dist/ directory */
...
server.resource(
  ...
  { mimeType: 'text/plain' },
  async () => {
    const content = resolveDistFile('debugger-api.md')
    return {
...
server.resource(
  ...
  { mimeType: 'text/plain' },
  async () => {
    const content = resolveDistFile('editor-api.md')
    return {
...
server.resource(
  ...
  { mimeType: 'text/plain' },
  async () => {
    const content = resolveDistFile('styles-api.md')
    return {
...
export async function startMcp(options: { host?: string; token?: string } = {}) {
  if (options.host) {
    ...
    process.env.RUNBROWSER_HOST = options.host // backward compat
  }
  if (options.token) {
    process.env.RUNBROWSER_TOKEN = options.token
    process.env.RUNBROWSER_TOKEN = options.token // backward compat
  }
