...
async function ensureGhostCursorInjected(options: { page: Page }): Promise<void> {
  ...
  const hasGhostCursor = await page.evaluate(() => {
    return Boolean((globalThis as { __runbrowserGhostCursor?: unknown }).__runbrowserGhostCursor)
  })
...
export async function enableGhostCursor(options: {
  ...
  await page.evaluate(
    ({ optionsFromNode }) => {
      const api = (globalThis as { __runbrowserGhostCursor?: GhostCursorBrowserApi }).__runbrowserGhostCursor
