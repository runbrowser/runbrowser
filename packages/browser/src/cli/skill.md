...
termio-browser eval '<js>'                  # JavaScript in the page (see caveat below)
...
termio-browser skill install                # write the skill into ./.claude and ./.agents
```

Both `cdp` and `eval` read from stdin when the payload is long:

```bash
termio-browser cdp Runtime.evaluate <<'JSON'
...
```bash
termio-browser cdp Target.getTargets | jq '.targetInfos[] | select(.type=="page") | .url'
```

`eval` prints the value as plain text, not JSON — `termio-browser eval 'document.title'`
...
```bash
termio-browser eval 'document.title'    # → the title
termio-browser eval 'document.title;'   # → empty. The trailing ; changed the semantics.
```
...
```bash
termio-browser eval 'const t = document.title; return t.toUpperCase()'
```
...
```bash
termio-browser eval 'document.querySelector("main").innerText.slice(0, 2000)'
```
...
```bash
termio-browser eval 'document.querySelector("#submit").click()'
```
...
termio-browser cdp DOM.scrollIntoViewIfNeeded '{"backendNodeId": 1234}'
termio-browser cdp DOM.getBoxModel '{"backendNodeId": 1234}'
# centre of the content quad: [x1,y1, x2,y2, x3,y3, x4,y4]
termio-browser cdp Input.dispatchMouseEvent '{"type":"mouseMoved","x":420,"y":310}'
termio-browser cdp Input.dispatchMouseEvent '{"type":"mousePressed","x":420,"y":310,"button":"left","clickCount":1}'
termio-browser cdp Input.dispatchMouseEvent '{"type":"mouseReleased","x":420,"y":310,"button":"left","clickCount":1}'
```
...
```bash
termio-browser cdp DOM.focus '{"backendNodeId": 1234}'
termio-browser cdp Input.insertText '{"text":"hello"}'
termio-browser cdp Input.dispatchKeyEvent '{"type":"keyDown","key":"Enter","code":"Enter","windowsVirtualKeyCode":13}'
termio-browser cdp Input.dispatchKeyEvent '{"type":"keyUp","key":"Enter","code":"Enter","windowsVirtualKeyCode":13}'
```
...
```bash
until [ "$(termio-browser eval 'document.readyState')" = complete ]; do sleep 0.2; done
```

For SPAs, `readyState` lies — poll for the thing you actually need:

```bash
until termio-browser eval 'document.querySelectorAll(".item").length >= 10' | grep -q true; do sleep 0.3; done
```
...
termio-browser tab list                      # → marks the tab you're bound to
termio-browser tab 2                         # bind to tab 2
```
