...
export const RELAY_PORT = Number(process.env.RUNBROWSER_PORT || process.env.RUNBROWSER_PORT) || 19988
...
): Promise<{ connected: boolean; activeTargets: number; extensionVersion: string | null } | null> {
  try {
 ...
 */
export function getExtensionOutdatedWarning(extensionRunbrowserVersion: string | null | undefined): string | null {
  if (!extensionRunbrowserVersion) {
    return null
  }
