...
Una extensión de Chrome y un CLI que permiten a tus agentes controlar **tu navegador real** — con sesiones, extensiones y cookies ya disponibles. Sin instancia headless, sin detección de bots, sin memoria extra. [Star en GitHub](https://github.com/yuanjiwei/runbrowser).
...
Instala **comandos mantenidos por la comunidad** del repositorio [runbrowser/commands](https://github.com/runbrowser/commands) con un solo comando. Sin clonar, sin pasos de construcción — instala y usa.

```bash
# Listar paquetes de comandos disponibles
runbrowser install --list
...
**Cómo funciona:** El CLI descarga archivos `.ts` del repositorio GitHub `runbrowser/commands` y los guarda localmente. El servidor relay los carga vía jiti en tiempo de ejecución — sin compilación necesaria. Cada paquete es un directorio con uno o más archivos de comandos.

**Contribuye tus propios comandos:** Crea un directorio en el repositorio [runbrowser/commands](https://github.com/runbrowser/commands) con tus archivos `.ts` y envía un PR. Tus comandos estarán disponibles para todos vía `runbrowser install`.

<Caption>Instalación con un comando. Mantenido por la comunidad. Mismo formato TypeScript que los comandos personalizados.</Caption>
...
<ComparisonTable
  ...
  rows={[
    ['Herramientas', '12+ herramientas MCP', '2 herramientas (skill + run)'],
    ['API', 'Acciones limitadas', 'Acceso CDP completo'],
    ['Uso de contexto', 'Alto (esquemas)', 'Bajo'],
