...
Installez des **commandes maintenues par la communauté** depuis le dépôt [runbrowser/commands](https://github.com/runbrowser/commands) avec une seule commande. Pas de clonage, pas d'étape de build — installez et utilisez.

```bash
# Lister les paquets de commandes disponibles
...
**Fonctionnement :** Le CLI récupère les fichiers `.ts` depuis le dépôt GitHub `runbrowser/commands` et les sauvegarde localement. Le serveur relais les charge via jiti à l'exécution — pas de compilation nécessaire. Chaque paquet est un répertoire avec un ou plusieurs fichiers de commandes.

**Contribuez vos propres commandes :** Créez un répertoire dans le dépôt [runbrowser/commands](https://github.com/runbrowser/commands) avec vos fichiers `.ts` et soumettez une PR. Vos commandes deviennent disponibles pour tous via `runbrowser install`.

<Caption>Installation en une commande. Maintenu par la communauté. Même format TypeScript que les commandes personnalisées.</Caption>
...
<ComparisonTable
  ...
  rows={[
    ['Outils', '12+ outils MCP dédiés', '2 outils (skill + run)'],
    ['API', 'Actions limitées', 'Accès CDP complet'],
    ['Utilisation contexte', 'Élevée (schémas)', 'Faible'],
