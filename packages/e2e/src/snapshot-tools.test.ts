...
// TODO Phase 6: AriaSnapshotNode, getCDPSessionForPage, screenshotWithAccessibilityLabels
// have been removed. These tests need to be updated.
import path from 'node:path'
import fs from 'node:fs'
import os from 'node:os'
import { imageSize } from 'image-size'
import { getCdpUrl } from '@jiweiyuan/runbrowser-server/utils'
import type { CDPCommand } from '@jiweiyuan/runbrowser-server/types'
...
describe('Snapshot & Screenshot Tests', () => {
  ...
  it('should capture screenshot correctly', async () => {
    ...
    const screenshotPath = path.join(os.tmpdir(), 'runbrowser-test-screenshot.png')
  ...
  it.skip('should get aria ref for locator using getAriaSnapshot (TODO Phase 6: rewrite using new MCP snapshot tool)', async () => {
    const browserContext = getBrowserContext()
    const serviceWorker = await getExtensionServiceWorker(browserContext)
  ...
  it.skip('should show aria ref labels on real pages and save screenshots (TODO Phase 6: rewrite using new MCP tools)', async () => {
    const browserContext = getBrowserContext()
    const serviceWorker = await getExtensionServiceWorker(browserContext)
    ...
    for (const { name, url, page } of pages) {
      ...
      const labelsAfterHide = await withTimeout(
        `verifyHide(${name})`,
        async () => {
          return await cdpPage.evaluate(() => document.getElementById('__runbrowser_labels__'))
        },
        10000,
      )
  ...
  it.skip('should take screenshot with accessibility labels via MCP execute tool (TODO Phase 6: rewrite using new MCP screenshot tool)', async () => {
    const browserContext = getBrowserContext()
    const serviceWorker = await getExtensionServiceWorker(browserContext)
