...
import type { CDPEvent, Protocol } from 'runbrowser/types'
import type { ExtensionCommandMessage, ExtensionResponseMessage, RelayToExtensionMessage } from 'runbrowser/protocol'
...
const RELAY_PORT = Number(process.env.RUNBROWSER_PORT) || 8790
...
function hasNumericId(value: Record<string, unknown>): value is Record<string, unknown> & { id: number } {
  return typeof value.id === 'number'
}

function hasOptionalSessionId(params: Record<string, unknown>): boolean {
  return params.sessionId === undefined || typeof params.sessionId === 'string'
}

function isRelayToExtensionMessage(value: unknown): value is RelayToExtensionMessage {
...
function getPerformanceMemory(): PerformanceMemoryInfo | undefined {
  return (performance as Performance & { memory?: PerformanceMemoryInfo }).memory
}
...
class ConnectionManager {
  ...
  private async connect(): Promise<void> {
    ...
    this.ws.onmessage = async (event: MessageEvent) => {
      ...
      try {
        ...
        sendMessage({ error: { code: -32700, message: `Error parsing message: ${message}` } })
        return
      }
  ...
  async maintainLoop(): Promise<void> {
    while (true) {
      ...
      // When another RunBrowser extension took over, poll until slot is free.
...
function sendLog(level: 'log' | 'debug' | 'info' | 'warn' | 'error', args: unknown[]) {
  sendMessage({
    method: 'log',
...
async function syncTabGroup(): Promise<void> {
  try {
    ...
    // If no connected tabs, clear any existing runbrowser groups
...
async function handleCommand(msg: ExtensionCommandMessage): Promise<unknown> {
  ...
  // if (msg.params.method === 'Network.enable' && msg.params.source !== 'runbrowser') {
  //   logger.debug('Skipping Network.enable from non-runbrowser CDP client:', msg.params.sessionId)
  //   return {}
  // }
...
async function connectTab(tabId: number): Promise<void> {
  try {
    ...
    const isExtensionInUse =
      ...
      error.message === 'Another RunBrowser extension is already connected'
...
async function onActionClicked(tab: chrome.tabs.Tab): Promise<void> {
  ...
  // If another RunBrowser extension took over, clear error state and try to reconnect this tab
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.groupId !== undefined) {
    ...
    tabGroupQueue = tabGroupQueue
      .then(async () => {
        // Query for runbrowser group by title - no stale cached ID
        const existingGroups = await chrome.tabGroups.query({ title: 'runbrowser' })
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'runbrowser-pin-element' || !tab?.id) return
