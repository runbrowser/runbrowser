import { getFrameId, isFrame } from "./playwright-compat.js"
...
import type { Page, Locator, ElementHandle, Frame, FrameLocator } from 'playwright-core'
...
export async function getAriaSnapshot({
  ...
  const frameId = getFrameId(resolvedFrame!) ?? null
...
export async function hideAriaRefLabels({ page }: { page: Page }): Promise<void> {
  await page.evaluate(() => {
    ...
    if (a11y) {
      ...
      const timerKey = '__runbrowser_labels_timer__'
      if (win[timerKey]) {
        win.clearTimeout(win[timerKey])
        win[timerKey] = null
      }
      doc.getElementById('__runbrowser_labels__')?.remove()
    }
  })
}
...
export async function screenshotWithAccessibilityLabels({
  ...
  const filename = `runbrowser-screenshot-${timestamp}-${random}.jpg`
  ...
  const buffer = await (async () => {
    if (!sharp) {
      logger?.error?.('[runbrowser] sharp not available, using clipped screenshot (max', LLM_MAX_DIMENSION, 'px)')
      return rawBuffer
    }
    try {
      ...
      logger?.error?.('[runbrowser] sharp resize failed, using raw buffer:', err)
      return rawBuffer
    }
  })()
  if (log) {
