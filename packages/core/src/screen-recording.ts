...
/**
 * Screen recording utility for runbrowser using chrome.tabCapture.
...
interface CreateRecordingApiOptions {
  ...
  defaultPage: Page
  relayPort: number
  cursorOverlayController: RecordingCursorOverlayController
  onStart: () => void
  onFinish: () => void
...
function resolveRecordingTargetPage(options: {
  ...
  return options.cursorOverlayController.resolveRecordingTargetPage({
...
function withRecordingDefaults<T extends { page?: Page; sessionId?: string }, R>(options: {
  ...
  return async (input: T = {} as T) => {
    ...
    const sessionId = input.sessionId || getSessionId(targetPage) || undefined
...
export function createRecordingApi(options: CreateRecordingApiOptions): {
  ...
  const { context, defaultPage, relayPort, cursorOverlayController, onStart, onFinish, getExecutionTimestamps } = options
  ...
  const isRecordingWithDefaults = async (opts: IsRecordingWithDefaultsOptions = {}): Promise<RecordingState> => {
    ...
    const sessionId = opts.sessionId || getSessionId(targetPage) || undefined
  ...
  const start = async (opts?: StartRecordingWithDefaultsOptions): Promise<RecordingState> => {
    const targetPage = resolveRecordingTargetPage({ context, defaultPage, cursorOverlayController, target: opts })
    const result = await startWithDefaults(opts)
    onStart()
    await cursorOverlayController.enableForRecording({ page: targetPage })
    return result
  }
  ...
  const stop = async (
    ...
    onFinish()
    await cursorOverlayController.disableForRecording({ page: targetPage })
    ...
    if (isActiveTabPermissionError(errorMsg)) {
      ...
      throw new Error(
        `Failed to start recording: ${errorMsg}\n\n` +
          ...
          `  ${restartCmd}\n\n` +
          `Or click the RunBrowser extension icon on the tab once to grant permission.`,
      )
    }
