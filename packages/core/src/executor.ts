import { getTargetId, getSelector } from "./playwright-compat.js"
/**
...
import { Page, Frame, Browser, BrowserContext, chromium, Locator, FrameLocator } from 'playwright-core'
...
import { getExtensionOutdatedWarning } from '@runbrowser/relay/client'
...
import { type CursorOverlayOptions } from './cursor-overlay.js'
import { RecordingCursorOverlayController } from './recording-cursor-overlay.js'
...
const EXTENSION_NOT_CONNECTED_ERROR = `The RunBrowser Chrome extension is not connected. Make sure you have:
1. Installed the extension: https://chromewebstore.google.com/detail/runbrowser-mcp/jfeammnjpkecdekppnclgkkffahnhfhe
2. Clicked the extension icon on a tab to enable it (or refreshed the page if just installed)`

const NO_PAGES_AVAILABLE_ERROR =
  'No Playwright pages are available. Enable RunBrowser on a tab or set RUNBROWSER_AUTO_ENABLE=1 to auto-create one.'
...
export class PlaywrightExecutor {
  ...
  private warnIfExtensionOutdated(extensionVersion: string | null) {
    if (this.hasWarnedExtensionOutdated) {
      return
    }
    const warning = getExtensionOutdatedWarning(extensionVersion)
    if (warning) {
  ...
  private setupPageCloseDetection(page: Page) {
    page.on('close', () => {
      ...
      this.enqueueWarning(
        `The current page in ${stateKeyLabel} was closed (url: ${closedUrl}). ` +
          `No open pages remain. Open a tab with RunBrowser enabled, then reassign ${stateKeyLabel}.`,
      )
    })
  }
    ...
    page.on('popup', (popup) => {
      ...
      this.enqueueWarning(
        `Popup window detected (page index ${pageIndex}, url: ${url}). ` +
          `Popup windows cannot be controlled by runbrowser. ` +
  ...
  private setupPageConsoleListener(page: Page) {
    ...
    const targetId = getTargetId(page) ?? ((page as any)._guid as string | undefined)
    if (!targetId) {
      return
    }
  ...
  private async checkExtensionStatus(): Promise<{
    ...
    const notConnected = { connected: false, activeTargets: 0, extensionVersion: null }
    try {
      if (extensionId) {
        ...
        return {
          ...
          extensionVersion: extension?.extensionVersion || null,
        }
      }
      ...
      return (await response.json()) as { connected: boolean; activeTargets: number; extensionVersion: string | null }
    } catch {
      return notConnected
    }
  }
  ...
  private async ensureConnection(): Promise<{ browser: Browser; page: Page }> {
    ...
    this.warnIfExtensionOutdated(extensionStatus.extensionVersion)
  ...
  async execute(code: string, timeout = 10000): Promise<ExecuteResult> {
    ...
    try {
      ...
      const snapshot = async (options: {
        ...
        // Pass a CDP session to avoid getCDPSessionForPage port detection issues
        const cdpForSnapshot = await getCDPSession({ page: resolvedPage })
        const {
        ...
        const snapshotKey = locator ? `locator:${getSelector(locator)}` : 'page'
      ...
      const getLatestLogs = async (options?: { page?: Page; count?: number; search?: string | RegExp }) => {
        ...
        if (filterPage) {
          // Use targetId() if available, fallback to internal _guid for CDP connections
          const targetId = getTargetId(filterPage) ?? ((filterPage as any)._guid as string | undefined)
          if (!targetId) {
      ...
      const getCDPSession = async (options: { page: Page }) => {
        ...
        return await getCDPSessionForPage({ page: options.page, port: this.cdpConfig.port })
      }
      ...
      const screenshotWithAccessibilityLabelsFn = async (options: { page: Page; interactiveOnly?: boolean }) => {
        return screenshotWithAccessibilityLabels({
          ...
          logger: {
            info: (...args) => {
              this.logger.error('[runbrowser]', ...args)
            },
            error: (...args) => {
              this.logger.error('[runbrowser]', ...args)
            },
      ...
      // This permission is granted when the user clicks the RunBrowser extension icon on a tab.
      const relayPort = this.cdpConfig.port || 19988
      const self = this
      const recordingCursorOverlay = new RecordingCursorOverlayController({
        logger: {
          error: (...args: unknown[]) => {
      ...
      const showCursorOverlay = async (options?: ({ page?: Page } & CursorOverlayOptions)) => {
        const targetPage = options?.page || page
        const cursorOptions: CursorOverlayOptions | undefined = (() => {
          if (!options) {
            return undefined
          }
        ...
        await recordingCursorOverlay.show({ page: targetPage, cursorOptions })
      }

      const hideCursorOverlay = async (options?: { page?: Page }) => {
        const targetPage = options?.page || page
        await recordingCursorOverlay.hide({ page: targetPage })
      }
      ...
      const recordingApi = createRecordingApi({
        ...
        defaultPage: page,
        relayPort,
        cursorOverlayController: recordingCursorOverlay,
        onStart: () => {
  ...
  // When extension is connected but has no pages, auto-create only if RUNBROWSER_AUTO_ENABLE is set.
  private async ensurePageForContext(options: { context: BrowserContext; timeout: number }): Promise<Page> {
    ...
    if (!process.env.RUNBROWSER_AUTO_ENABLE && !process.env.RUNBROWSER_AUTO_ENABLE) {
